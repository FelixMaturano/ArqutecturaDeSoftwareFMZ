...
<template>
  <div class="crud-container">
    ...
    <div class="lista-panel">
      ...
      <p v-if="categorias.length === 0" class="mensaje-vacio">Cargando datos...</p>
  ...
  methods: {
    ...
    async eliminarCategoria(id) {
      if(confirm("¿Seguro que deseas borrar esta categoría?")) {
        try {
          await fetch(`http://localhost:3005/api/v1/categories/${id}`, { method: "DELETE" });
          this.leerCategorias(); 
        } catch (error) {
